      ...
      chrome.tabs.sendMessage(tab.id, 'getURL', async function(response) {
        if (response) {
          ...
          try {
            ...
            // Summarization API
            const summarizationUrl = 'https://text-summarize-pro.p.rapidapi.com/summarizeFromText';
            const summarizationOptions = {
                method: 'POST',
                headers: {
                    'content-type': 'application/x-www-form-urlencoded',
                    'X-RapidAPI-Key': '9097ecbcbfmshbd3252b8ada3ee2p139094jsne2bc3d0c3624',
                    'X-RapidAPI-Host': 'text-summarize-pro.p.rapidapi.com'
                },
                body: new URLSearchParams({
                    text: newsCombinedString,
                    percentage: '82'
                })
            };

            try {
                const summarizationResponse = await fetch(summarizationUrl, summarizationOptions);
                const summarizationResult = await summarizationResponse.json();
                const summarizationSummary = summarizationResult.summary;
                console.log(summarizationSummary)
                ...
                const sentimentOptions = {
                    ...
                    body: JSON.stringify({ text: summarizationSummary })
                };
                ...
                try {
                    ...
                    let averageSentimentScore = value;
                    if (sentiment === 'positive') {
                        averageSentimentScore = 50 + 50 * value;
                    ...
                    document.getElementById('apiResult').textContent = "Error fetching sentiment data from the API.";
                }
